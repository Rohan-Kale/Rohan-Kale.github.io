...
const external = (value, label) => value ? `<a class="text-link" href="${url(value)}" target="_blank" rel="noopener noreferrer">${label} <span aria-hidden="true">↗</span><span class="sr-only"> (opens in a new tab)</span></a>` : `<span class="unavailable">${label} — not added</span>`;
...
const replacements = {
  ...
  RESUME: data.resume ? `<a class="homepage-link" href="${data.resume.startsWith('/files/') ? `.${esc(data.resume)}` : url(data.resume)}" target="_blank" rel="noopener noreferrer">Resume <span aria-hidden="true">↗</span><span class="sr-only"> (opens in a new tab)</span></a>` : '<a class="homepage-link" href="./resume-placeholder.html">Resume <span aria-hidden="true">↗</span><small>Not added yet</small></a>',
  ...
  PROJECTS: projects,
  SATELLITES: data.projects.map((p, i) => `<a href="#${esc(p.id)}" class="satellite-link"><span aria-hidden="true">${String(i + 1).padStart(2, '0')}</span>${esc(p.title)}<span aria-hidden="true">↗</span></a>`).join(''),
